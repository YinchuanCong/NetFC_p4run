...
const int<16> scaling_factor = 256;
//-------------------------------------------------------------------------------       Headers------------------------------//
...
control ingress(inout header_t hdr, inout metadata_t ig_md, inout standard_metadata_t standard_metadata){
    ...
    apply {
        ...
        ig_md.ac_md.sign =   (hdr.calc.x ^ hdr.calc.y)&0x8000;

        if(hdr.calc.x[15:15] == hdr.calc.y[15:15]){
            ig_md.ac_md.sign = 0x0000;
        }else{
            ig_md.ac_md.sign = 0x8000;
        }
        ...
        get_log_i_table.apply(); // int(log(x))
        get_log_j_table.apply(); // int(log(y))

        ig_md.ac_md.log_k = (bit<16>)((int<16>)ig_md.ac_md.log_i - (int<16>)ig_md.ac_md.log_j);

        // if(ig_md.ac_md.log_i > ig_md.ac_md.log_j){
        //     ig_md.ac_md.log_k = (bit<16>)((int<16>)ig_md.ac_md.log_i - (int<16>)ig_md.ac_md.log_j);
        // }else{
        //     ig_md.ac_md.log_k = ((bit<16>)((int<16>)ig_md.ac_md.log_j - (int<16>)ig_md.ac_md.log_i))|0x8000;
        // }

       
        if(ig_md.ac_md.log_k & 0x8000 == 0){

            if(ig_md.ac_md.log_k > 0x0F00){  // 18 * 256 = 0x0F00, 256 is the scaling factor
            hdr.calc.z = (bit<16>)0b0111110000000000;
            ig_md.ac_md.d_info.z=hdr.calc.z;
            digest<digest_info>(1,ig_md.ac_md.d_info);
            exit;
        }} 
        
        if(ig_md.ac_md.log_k & 0x8000 == 1){
        if(ig_md.ac_md.log_k&0x8000 > 0x0F00){
            hdr.calc.z = (bit<16>)0x0;
        ...
        ig_md.ac_md.n = ig_md.ac_md.log_k;
        get_abs_z_table.apply();
        hdr.calc.z = hdr.calc.z | ig_md.ac_md.sign;

        ig_md.ac_md.d_info.x=(bit<16>)hdr.calc.x;
        ig_md.ac_md.d_info.y=(bit<16>)hdr.calc.y;
        // ig_md.ac_md.d_info.z=(bit<16>)ig_md.ac_md.log_k;
        ig_md.ac_md.d_info.z = hdr.calc.z;
