...
class L2Controller(object):
    ...
    def unpack_digest(self, msg, num_samples):
        ...
        for sample in range(num_samples):
            ...
            print(msg[starting_index:starting_index+6])
            ...
            print(struct.unpack(">H",msg[starting_index:starting_index+2])[0])
            print(struct.unpack(">h",msg[starting_index:starting_index+2])[0])            
            
            print('y:',np.float32(y),end=' ')
            print(''.join(format(by,'08b') for by in struct.pack('!e',y)))
            print(struct.unpack(">H",msg[starting_index+2:starting_index+4])[0])
            print(struct.unpack(">h",msg[starting_index+2:starting_index+4])[0])
            
            
            print('z:',np.float32(z),end=' ')
            print(''.join(format(by,'08b') for by in struct.pack('!e',z)))
            print(struct.unpack(">H",msg[starting_index+4:starting_index+6])[0])
            print(struct.unpack(">h",msg[starting_index+4:starting_index+6])[0])
