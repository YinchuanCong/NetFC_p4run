...
control ingress(inout header_t hdr, inout metadata_t ig_md, inout standard_metadata_t standard_metadata){
    ...
    apply {
        ...
        // ig_md.ac_md.d_info.z=ig_md.ac_md.sign;

        // digest<digest_info>(1,ig_md.ac_md.d_info);
        if(ig_md.ac_md.log_k & 0x8000 == 0){

            if(ig_md.ac_md.log_k > 0x0F00){  // 0x0F00= 18 * 256, 256 is the scaling factor
        ...
        if((int<16>)(ig_md.ac_md.log_k) < (int<16>)0b1110111000000000){
        ...
        ig_md.ac_md.d_info.x=(bit<16>)ig_md.ac_md.log_i;
